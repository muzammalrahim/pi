...
class Xuser(models.Model):
    ...
    password = models.CharField(max_length=12, blank=True, null=True,
                                help_text="at least 8 long, one capital, one special character")
    ...
    def get_menu(self):
        if self.role == 'admin':
            sstr = {'/newrpi': 'new devices', '/users': 'users', '/settings': 'settings',
                    '/clicommands': 'cli commands'}
        else:
            sstr = {'/xrpis': 'my devices', '/xnewrpi': 'new device'}
        sstr['/myaccount'] = 'my account'
        sstr['/password'] = 'reset password'
        sstr['/'] = 'logout'
        return sstr
